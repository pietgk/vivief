...
**Concept vs. pattern.** The following are patterns, not concepts: domain, bridge, artifact, slice, profile. They compose the five concepts but don't extend them. Domain is the primary composition pattern — the most important way the five concepts combine for a user population.
...
*Version: v5 — trims conceptual surface area while sharpening concept boundaries. Deterministic-first elevated to core thesis with Contract↔effectHandler relationship formulation. Effects as understanding connects extraction, aggregation, observability, and the deterministic-first loop. Surface modes trimmed to 6, Projection profiles to 3, Cultural Contract absorbed into Guard Contract. Added: reading guide, Contract decision tree, developer walkthrough, operations as creation of understanding, `:creation/abandoned` state. Glossary grouped by concept (~30 entries). Model references use capability categories. Domains explicitly exclusive. Infrastructure, portability tables, and detailed bridge mappings moved to implementation KB. Five concepts remain sufficient. Replaces v4.*
