by Jack Herrington, Alem Tuzlak, and Tanner Linsley on Apr 8, 2026.
...
Frontier LLMs are very good at writing TypeScript. They have an enormous amount of real-world TypeScript in their training data. They know how to Promise.all, .reduce(), .filter(), and handle async control flow.
...
The N+1 example
Consider an e-commerce app where the user asks: "What are the top 5 best-selling products, and what's the average rating for each?"
...
typescriptconst top = await external_getTopProducts({ limit: 5 })
...
Setting It Up
Install
shpnpm add @tanstack/ai @tanstack/ai-code-mode zodpnpm add @tanstack/ai @tanstack/ai-code-mode zod
Pick an isolate driver:
sh# Node.js: fastest, V8 isolates via isolated-vm
...
pnpm add @tanstack/ai-isolate-cloudflare# Node.js: fastest, V8 isolates via isolated-vm
pnpm add @tanstack/ai-isolate-node
...
typescriptimport { toolDefinition } from '@tanstack/ai'
import { z } from 'zod'
...
typescriptimport { chat } from '@tanstack/ai'
...
DriverBest forNative depsBrowser@tanstack/ai-isolate-nodeServer-side Node.jsYes (C++ addon)No@tanstack/ai-isolate-quickjsBrowsers, edge, portabilityNone (WASM)Yes@tanstack/ai-isolate-cloudflareEdge on CloudflareNoneN/A
...
typescriptimport { codeModeWithSkills } from '@tanstack/ai-code-mode-skills'
...
StrategyProvisional atTrusted atDefault10+ runs, ≥90% success100+ runs, ≥95% successRelaxed3+ runs, ≥80% success10+ runs, ≥90% successAlways trustedImmediatelyImmediatelyCustomYou decideYou decide
...
EventWhencode_mode:execution_startedSandbox begins executingcode_mode:consoleEach console.log/error/warn/infocode_mode:external_callBefore an external_* function runscode_mode:external_resultAfter a successful external_* callcode_mode:external_errorWhen an external_* call fails
...
To run it:
shgit clone https://github.com/TanStack/ai.git
cd ai
pnpm install
pnpm --filter ts-code-mode-web devgit clone https://github.com/TanStack/ai.git
cd ai
pnpm install
...
shpnpm add @tanstack/ai-code-mode                # Core
pnpm add @tanstack/ai-code-mode-skills         # Persistent skills
pnpm add @tanstack/ai-isolate-node             # or quickjs, or cloudflarepnpm add @tanstack/ai-code-mode                # Core
pnpm add @tanstack/ai-code-mode-skills         # Persistent skills
...
The full documentation is in the Code Mode Guide and the Skills Guide.

TanStack AI is open-source, provider-agnostic, and framework-agnostic. Get started here.
