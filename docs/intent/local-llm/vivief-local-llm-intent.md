...
### Performance on M1 Pro 32GB (Community Benchmarks)

| Model | Ollama tok/s | Memory |
|-------|-------------|--------|
| E2B | ~95 | ~3.6 GB |
| E4B | ~57 | ~5.5 GB |
| 26B MoE | ~2 (unusable) | swaps |
| 31B | won't fit | — |

**Conclusion**: E2B and E4B are the viable local models on current hardware. The 26B/31B are API or server-only for us.
...
### Four-Tier Model (was three)

| Tier | Model | Size | Purpose | Latency | Runtime |
|------|-------|------|---------|---------|---------|
| **Micro** | VivRouter (FunctionGemma fine-tune) | 270M | Intent classification, primitive routing, quick validation | <100ms | LiteRT-LM |
| **Local** | VivSurface / VivPractitioner (E4B + LoRA) | 4B | Surface compilation, diagram gen, function calling, domain reasoning (thinking mode) | 1-3s | LiteRT-LM |
