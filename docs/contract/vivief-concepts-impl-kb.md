# Vivief Concepts — Implementation Knowledge Base (v6)
...
**What this document is.** A decision framework that maps vivief-concepts-v6.md to implementation choices. Each technology area is framed as an open decision with criteria derived from the concepts, candidate approaches, and tradeoffs. The format is decision-record style: What to decide / Why it matters / Criteria / Candidates / Concept connection.
...
**Relationship to vivief-concepts-v6.md.** The concepts document defines WHAT the platform does and WHY. This document frames HOW to get there — without prescribing answers. The concepts are stable across technology choices; the technology choices serve the concepts.
...
**Skills as implementation strategy.** The effectHandler implementation strategy dimension (v6 §2.5) recognizes that effectHandlers can be implemented in markdown and executed by an LLM runtime. This is what Claude Code skills are today — markdown files that orchestrate multi-step workflows. The vivief generalization adds trust scoring, Contract governance, datom-native storage, lifecycle management, and the deterministic-first evolution path.

Skills are NOT a cross-LLM standard — each platform has its own format (Claude Code skills, OpenAI GPT actions, LangChain agents). MCP is the cross-LLM tool layer. The orchestration layer (skill) is inherently platform-specific. Portability efforts focus on ensuring effectHandlers are exposable via MCP (cross-LLM) and CLI (universal), not on standardizing skill formats.
...
| **Contract** | Validation runtime (pre-commit/in-flight/post-commit hooks), state machine engine, aggregation pipeline, enforcement strategy wiring (external/internal per Contract type) |
| **effectHandler** | Execution runtime (sync functions + async actors), message queue, failure handling, actor lifecycle management, implementation strategy support (code + LLM + StateMachine + hybrid) |
...
| Phase | What | Depends on | v6 concept |
|-------|------|-----------|------------|
| 1 | **Datom store** with provenance attributes | — | Datom (§2.1) |
| 2 | **Schema Contract** validation at commit | Phase 1 | Contract pre-commit, external enforcement (§2.4) |
...
| 7 | **effectHandler actor** + streaming + **in-flight Contract as system actor** | Phase 4, 6 | effectHandler Level 2, in-flight enforcement (§2.4, §2.5) |
...
| 12 | **Behavior Contract** + StateMachine mode + **internal enforcement** | Phase 4 | Contract modes, enforcement strategy (§2.4) |
...
### effectHandler Exposure Wrapping

An effectHandler is made available to different consumers via wrappers that adapt protocol, not logic. The effectHandler is unchanged — the wrapper makes it invocable.

**Thin wrappers** adapt protocol:

| Wrapper | Consumer | What it adds |
|---------|----------|-------------|
| **MCP tool** | Any LLM agent | Protocol envelope, tool schema — cross-LLM |
| **CLI command** | Human or any LLM with shell | Argument parsing, stdout — universal |

**Thick wrappers** compose effectHandlers:

| Wrapper | Consumer | What it adds |
|---------|----------|-------------|
| **Skill** | Specific LLM runtime | Multi-step orchestration, context gathering — platform-specific |

A skill is the one wrapper type that adds orchestration — it composes multiple effectHandlers into a workflow, not just adapts a single one for a protocol. The underlying effectHandlers remain portable via thin wrappers.

**The devac reference architecture demonstrates this separation:**
...
Contract enforcement follows the trust boundary (v6 §2.4). Implementation approach per strategy:

**External enforcement (infrastructure):**
- Schema Contract → synchronous validation in the store's write path. Fast path for the common case (valid datoms).
- Trust Contract → key derivation and access control layer. System concern, handler cannot bypass.
- Sync Contract → conflict resolution in the replication layer. Handler concern is data, not resolution.
- Render Contract → external tooling (axe scanning, Storybook stories). Component cannot skip its own a11y check.
- In-flight Contract → **system actor** sitting between producer and consumer in the streaming pipeline. Validates each chunk against registered Contracts before it reaches the Surface. Rejection produces error datoms without stopping the stream.

**Internal enforcement (handler):**
- Behavior Contract (StateMachine) → the handler IS the constraint. XState validates transitions inside the handler. The machine definition is both the Contract and the enforcement.
...
Knowledge matures through a predictable path from tacit understanding to infrastructure enforcement:
...
This is the deterministic-first loop already running: LLM-discovered patterns written as rules, referenced by skills, improving outcomes measurably. When vivief has a working Contract system, these knowledge files become the first candidates for formalization: M1-M4 → Behavior Contract on documentation effectHandlers, G1-G4 → Behavior + Aggregation Contract on architecture effectHandlers.

### The Compounding Flywheel in Practice

A concrete example of the compounding flywheel (v6 §3) applied to a counselor's workflow:
...
Each week of skill use feeds the deterministic-first loop. The skill becomes more trustworthy. The Contracts it births apply to other handlers too. The whole system learns.
...
| **Formal enforcementTrustThreshold field** | Principle sufficient for now; field adds complexity | When mechanical enforcement routing proves necessary |

---

*Version: v6 implementation KB — decision framework for vivief platform implementation. Adds: effectHandler exposure wrapping as architecture pattern (thin wrappers adapt protocol, thick wrappers compose); enforcement strategy architecture with external/internal/post-commit patterns and in-flight enforcer as system actor; knowledge evolution path with devac M1-M4 evidence; compounding flywheel Monday prep example; skills as implementation strategy in AI/LLM section. Phase 7 updated to note in-flight enforcer is system actor. Phase 12 updated to note internal enforcement. enforcementTrustThreshold deferred. Replaces v5 implementation KB.*
