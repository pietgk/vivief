...
```typescript
type Effect =
  | { type: "InvokeHandler", handler: string, data: any }
  | { type: "DBRead", query: Query }
  | { type: "DBWrite", table: string, data: any }
  | { type: "HTTPRequest", url: string, method: string }
  | { type: "Navigate", url: string }
  | { type: "Render", component: Component }
  | { type: "FileChanged", path: string }
  | { type: "ValidationFailed", check: string, errors: Error[] }
  // ... extensible
```

### 5.3 Effect Hierarchies
...
- DevAC implementation in `vivief/`

---

*Version: 2.0 - Integrated VVE concepts from architecture packages*
