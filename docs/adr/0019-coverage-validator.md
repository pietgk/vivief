...
DevAC Phase 2 provides validation feedback for type errors (tsc), lint issues (eslint), and test results. However, test coverage—a key metric for code quality—was not integrated into the feedback loop.
...
2. **Hub Integration**: Coverage issues pushed to `unified_feedback` with source `"coverage"` and category `"testing"`
3. **IssueEnricher**: Coverage issues enriched with CodeGraph context (callers, dependents)
...
- Coverage issues visible via MCP tools (`get_all_feedback --source coverage`)
...
- Additional Hub storage for coverage issues

### Neutral

- Coverage issues are warnings, don't fail overall validation
...
| `devac-core/src/validation/hub-integration.ts` | Push coverage issues to Hub |
| `devac-core/src/hub/hub-storage.ts` | Added "coverage" to FeedbackSource |
| `devac-core/src/hub/central-hub.ts` | Handle coverage in pushValidationErrors |
...
```
get_all_feedback --source coverage
```
...
- [ADR-0017: Validation Hub Cache](0017-validation-hub-cache.md) - Validation storage architecture
- [ADR-0018: Unified Feedback Model](0018-unified-feedback-model.md) - Unified feedback schema
