...
describe("DevacMCPServer", () => {
  ...
  describe("initialization", () => {
    ...
    it("has correct tool count in status before start", () => {
      ...
      expect(status.toolCount).toBe(20); // find_symbol, get_dependencies, get_dependents, get_file_symbols, get_affected, get_call_graph, query_sql, get_schema, list_repos, get_context, get_validation_errors, get_validation_summary, get_validation_counts, get_all_diagnostics, get_diagnostics_summary, get_diagnostics_counts, query_effects, run_rules, list_rules, generate_c4
    });
  });
  ...
  describe("getStatus()", () => {
    it("returns complete status object", async () => {
      ...
      expect(status.packagePath).toBe(packagePath);
      expect(status.toolCount).toBe(20);
