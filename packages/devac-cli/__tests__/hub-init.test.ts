...
describe("hub init command", () => {
  ...
  it("creates ~/.devac/ directory if not exists", async () => {
    const result = await hubInit({ hubDir, skipValidation: true });
  ...
  it("overwrites existing hub with --force", async () => {
    ...
    const result = await hubInit({ hubDir, force: true, skipValidation: true });
  ...
  it("accepts custom hub directory path", async () => {
    ...
    const result = await hubInit({ hubDir: customDir, skipValidation: true });
  ...
  it("returns error if directory cannot be created", async () => {
    ...
    const result = await hubInit({ hubDir: invalidDir, skipValidation: true });
